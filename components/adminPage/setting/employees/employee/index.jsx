...
import { employeeCreatePost, resetemployee } from '../../../../../redux/sliceAdmin/employees/employee/teacheeCreate/index.js'
...
export const EmployeeComponent = () => {
    ...
    const employeeCreate = useSelector((store) => store.employeeCreate);
    ...
    useEffect(() => {
        if (employeeCreate.status === 'success') dispatch(startMessage({ time: 3, message: 'Muvofiyaqatli Yakulandi', type: 'success' })), setDataPush({
            ...dataPush,
            firstName: '',
            lastName: '',
            patron: '',
        ...
        else if (employeeCreate.status === 'notFound') dispatch(startMessage({ time: 3, message: employeeCreate.message.split('_').join(' ') }))
        setTimeout(() => { dispatch(resetemployee()) }, 500);
    }, [employeeCreate])
    ...
    useEffect(() => {
        if ((employeeCreate.status === 'success' || examdeleteId.status === 'success'))
            dispatch(getAllexamsubjectFetch())
    }, [employeeCreate, examdeleteId])
    ...
    const addFacultet = () => dispatch(employeeCreatePost({
